/**
 * P2PKH (Pay To Public Key Hash) 脚本模板
 * 
 * 本类提供创建 Pay To Public Key Hash 锁定和解锁脚本的方法，包括使用私钥解锁 P2PKH UTXO。
 */
...
export default class P2PKH implements ScriptTemplate {
   ...
   * @returns {LockingScript} - P2PKH 锁定脚本
   */
  lock(pubkey: PublicKey): LockingScript {
    // 获取公钥的哈希值
    const data = pubkey.toHash() as number[]
  ...
  /**
   * 创建一个函数，用于生成 P2PKH 解锁脚本及其签名和长度估计
   *
   * 返回的对象包含：
   * 1. `sign` - 一个函数，当使用交易和输入索引调用时，生成适用于 P2PKH 锁定输出的解锁脚本
   * 2. `estimateLength` - 一个函数，返回解锁脚本的估计长度（字节）
   *
   * @param {PrivateKey} privateKey - 用于签名交易的私钥
   * @param {number} sigHashType - 签名哈希类型，例如 TransactionSignature.SIGHASH_ALL | TransactionSignature.SIGHASH_FORKID
  ...
  unlock(
    ...
    return {
      sign: async (tx: Transaction, inputIndex: number) => {
        const signatureScope = sigHashType;
