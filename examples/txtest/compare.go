...
// matches Step1Hex, Step2Hex, Step3Hex
var stepRegexp = regexp.MustCompile(`Step([123])Hex\s+([0-9a-fA-F]+)`)
...
func capture(cmd *exec.Cmd) ([]string, error) {
	...
	return []string{string(matches[0][2]), string(matches[1][2]), string(matches[2][2])}, nil
}
