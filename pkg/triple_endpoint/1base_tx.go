...
func BuildTripleFeePoolBaseTx(
	...
	aUnlockingScriptTemplate, err := p2pkh.Unlock(aPrivateKey, &sigHash)
	if err != nil {
	...
	// 创建初始交易的锁定脚本
	outputMultisigScript, err := libs.Lock([]*ec.PublicKey{serverPublicKey, aPrivateKey.PubKey(), bPublicKey}, 2)
	if err != nil {
